...
#
# wasmaqua — a Wayland-inspired compositor with a macOS-Aqua-themed window
# decoration (three left-anchored traffic-light buttons, gradient titlebar),
...
#
# Forked from wasmdesk/wasmbox (BSD-3). The external-client wire protocol is
# IDENTICAL to wasmbox — only the Theme module + Window decoration paint
# differ, so the same hello / dock / quake / terminal / files wasm clients
# work unmodified.
#
...
# ---------------------------------------------------------------------------
# Theme — macOS Aqua decorations.
#
# Three colored "traffic-light" buttons (close/min/max) sit on the LEFT side
# of a 28px titlebar painted with a top-to-bottom gradient (TITLE_ACTIVE →
# TITLE_ACTIVE_2). An inactive window flattens the bar to TITLE_INACTIVE and
# greys the title text + the buttons. A 1px BORDER bottom edge separates the
# bar from the body. Window body keeps the same external-protocol blit path
# as wasmbox; only the decoration is themed.
# ---------------------------------------------------------------------------
module Theme
  DESKTOP        = "#1f1f23"
  DESKTOP_GRID   = "#26262b"
  TITLE_ACTIVE   = "#ECECEC"   # gradient top
  TITLE_ACTIVE_2 = "#D6D6D6"   # gradient bottom
  TITLE_INACTIVE = "#E3E3E3"
  TITLE_BORDER   = "#BFBFBF"
  TITLE_TEXT_ON  = "#3C3C43"
  TITLE_TEXT_OFF = "#9B9B9F"
  TITLE_TEXT     = TITLE_TEXT_ON # legacy alias for code that reads Theme::TITLE_TEXT
  CLOSE_RED      = "#FF5F57"
  ...
  BTN_GLYPH      = "#3C3C43"   # × / − / + ink (drawn faintly on hover)
  BORDER         = "#BFBFBF"   # legacy stroke colour used by the frame stroke
  BORDER_ACTIVE  = "#A9A9A9"
  BORDER_INACTIVE= "#CFCFCF"
  CLOSE_BG       = "#E6E7EE"   # kept for any legacy reader
  CLOSE_GLYPH    = "#3C3C43"
  RESIZE_GRIP    = "#9B9B9F"
  HUD_TEXT       = "#9aa0b4"
  MENU_BG        = "#FAFAFA"
  MENU_BORDER    = "#BFBFBF"
  MENU_TEXT      = "#1f1f23"
  MENU_HILITE    = "#0A60FF"   # macOS-style blue selection band
  SHADOW         = "#999999"   # 1px faked drop-shadow band on right/bottom

  TITLE_H        = 28          # titlebar height
  BORDER_W       = 1           # decoration border width
  BTN_R          = 6           # traffic-light button radius (px)
  BTN_DIAM       = BTN_R * 2   # = 12
  BTN_GAP        = 8           # spacing between buttons / from left edge
  CLOSE_SZ       = BTN_DIAM    # legacy alias (hit-box size)
  MIN_SZ         = BTN_DIAM    # legacy alias
  MAX_SZ         = BTN_DIAM
...
class Window
  ...
  # Pre-zoom rectangle stash for the green (maximize) traffic light: when set,
  # the window is currently zoomed and a second click on green restores the
  # window to this rect. nil = not currently zoomed.
  attr_accessor :prev_rect
  ...
  # macOS-Aqua close-rect: traffic-light button on the LEFT side of the
  # titlebar, 8px from the left edge, vertically centered. Hit-box is the
  # square circumscribing the BTN_DIAM-wide circle.
  def close_rect
    return [@x, @y, 0, 0] unless decorated?
    bx = @x + Theme::BTN_GAP
    by = frame_top + (Theme::TITLE_H - Theme::BTN_DIAM) / 2
    [bx, by, Theme::BTN_DIAM, Theme::BTN_DIAM]
  end

  # Minimize-rect (yellow) sits to the RIGHT of close, BTN_GAP apart.
  def minimize_rect
    return [@x, @y, 0, 0] unless decorated?
    cx, cy, _cw, _ch = close_rect
    [cx + Theme::BTN_DIAM + Theme::BTN_GAP, cy, Theme::BTN_DIAM, Theme::BTN_DIAM]
  end

  # Maximize-rect (green) sits to the RIGHT of minimize, BTN_GAP apart. The
  # behaviour wired to a hit is a no-op stub in v0 (zoom-toggle is roadmap).
  def maximize_rect
    return [@x, @y, 0, 0] unless decorated?
    mx, my, _mw, _mh = minimize_rect
    [mx + Theme::BTN_DIAM + Theme::BTN_GAP, my, Theme::BTN_DIAM, Theme::BTN_DIAM]
  end
  ...
  def on_maximize?(px, py)= decorated? ? hit?(maximize_rect, px, py) : false
  ...
  # Toggle macOS-style "zoom" (green button). Not zoomed → stash the current
  # body rect and expand to fill the canvas underneath the titlebar. Already
  # zoomed → restore to the stashed rect. canvas_w/canvas_h are the viewport
  # extent the Compositor passes in. No-op on undecorated surfaces.
  def zoom_toggle(canvas_w, canvas_h)
    ...
    if @prev_rect
      ...
      @x = 0
      @y = Theme::TITLE_H
      @w = [canvas_w, Theme::MIN_W].max
      @h = [canvas_h - Theme::TITLE_H, Theme::MIN_H].max
    end
  end
...
module RootMenu
  ...
  def self.build(wm)
    Menu.new([
      ...
      { separator: true },
      { label: "About wasmaqua", action: [:noop, "about"] },
...
class Compositor
  ...
  def on_mousedown(e)
    ...
    if win.on_close?(mx, my)
      ...
      @wm.minimize(win)
      notify_windows_changed
    elsif win.on_maximize?(mx, my)
      # macOS-Aqua green button: zoom-toggle. The pre-zoom rect lives on the
      # Window so a second click restores it.
      win.zoom_toggle(@width, @height)
      notify_windows_changed
  ...
  def draw_window(win)
    ...
    # 1px hairline at the bottom of the titlebar to separate it from the body.
    @ctx.set("strokeStyle", Theme::TITLE_BORDER)
    @ctx.set("lineWidth", 1)
    @ctx.call("beginPath")
    @ctx.call("moveTo", tx,        ty + Theme::TITLE_H - 0.5)
    @ctx.call("lineTo", tx + tw,   ty + Theme::TITLE_H - 0.5)
    @ctx.call("stroke")

    # Centered title text. Use textAlign=center so we can drop the title at
    # the bar midline without measuring the string ourselves.
    @ctx.set("font", "12px -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Helvetica, Arial, sans-serif")
    @ctx.set("textAlign", "center")
    @ctx.set("textBaseline", "middle")
    @ctx.set("fillStyle", active ? Theme::TITLE_TEXT_ON : Theme::TITLE_TEXT_OFF)
    @ctx.call("fillText", win.title, tx + tw / 2, ty + Theme::TITLE_H / 2 + 1)
    # Reset to the defaults the rest of the compositor assumes (left/alphabetic).
    @ctx.set("textAlign", "left")
    @ctx.set("textBaseline", "alphabetic")

    # Three traffic-light buttons. Active = saturated colour + 1px darker
    # outline; inactive = uniform muted grey for all three (matches macOS).
    draw_traffic_light(win.close_rect,    Theme::CLOSE_RED,  Theme::CLOSE_RED_OUT,  active)
    draw_traffic_light(win.minimize_rect, Theme::MIN_YELLOW, Theme::MIN_YELLOW_OUT, active)
    draw_traffic_light(win.maximize_rect, Theme::MAX_GREEN,  Theme::MAX_GREEN_OUT,  active)
    ...
    # Faked 1px drop shadow on the right + bottom edges of the frame so the
    # window pops off the desktop without a full canvas filter.
    fx, fy, fw, fh = win.frame_rect
    fill_rect([fx + fw,     fy + 1, 1, fh], Theme::SHADOW)
    fill_rect([fx + 1, fy + fh,     fw, 1], Theme::SHADOW)
    # 1px decoration border around the whole frame.
    stroke_rect(win.frame_rect, active ? Theme::BORDER_ACTIVE : Theme::BORDER_INACTIVE, Theme::BORDER_W)
  end

  # Paint one traffic-light button: a filled circle inscribed in `rect`, with
  # a 1px slightly-darker outline for depth. When the window is inactive all
  # three buttons render as a muted neutral grey (matching macOS behaviour).
  def draw_traffic_light(rect, fill_colour, outline_colour, active)
    bx, by, bw, bh = rect
    cx = bx + bw / 2.0
    cy = by + bh / 2.0
    r  = Theme::BTN_R
    ...
    @ctx.call("beginPath")
    @ctx.call("arc", cx, cy, r, 0, 6.283185307179586)
    @ctx.call("fill")
    @ctx.set("strokeStyle", oc)
    @ctx.set("lineWidth", 1)
    @ctx.call("beginPath")
    @ctx.call("arc", cx, cy, r - 0.5, 0, 6.283185307179586)
    @ctx.call("stroke")
  end
